...
class model2(nn.Module): #resnet model
    def __init__(self, num_classes=8, drop_prob=0.5):
        super(model2, self).__init__()
        self.n_classes = num_classes 

        # encoder
        self.model_ft = resnet34(pretrained=True) #used resnet34 here!
        ...
        self.cls_fc = nn.Linear(128, self.n_classes)
    ...
    def fine_tune(self, block_layer=5):
        for idx, child in enumerate(list(self.model_ft.children())[:-1]):  # Exclude the last fully connected layer
            if idx <= block_layer:
