...
from image_dataloader3 import image_loader3
from sklearn.metrics import confusion_matrix, accuracy_score

# load external modules
from utils import *
from image_dataloader3 import *
...
def main():
    ...
    parser.add_argument('--freeze_up_to', type=int, default=0, help='Specify up to which layer to freeze')
    ...
    train_dataset = image_loader3(data_dir=args.data_dir, label_file=args.label_file, split_file_path=args.split_file, train_flag=True, transforms=transforms)
    test_dataset = image_loader3(data_dir=args.data_dir, label_file=args.label_file, split_file_path=args.split_file, train_flag=False, transforms=transforms)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False)
    
    # Initialize the model
    #num_classes = 3
    model = model2(num_classes=8)
    
    model.to(device)
    if args.checkpoint:
        model.load_state_dict(torch.load(args.checkpoint))

    model.fine_tune(block_layer=args.freeze_up_to)
    ...
    if args.model_path:
        torch.save(model.state_dict(), os.path.join(args.model_path, 'model06-8.pth'))
        print(f'Training completed and model saved as {os.path.join(args.model_path, "model06-8.pth")}')
    else:
